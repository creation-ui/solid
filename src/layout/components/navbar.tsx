...
function NavbarMenu({
  ...
  return (
    <div class="nx-relative nx-inline-block">
      <Menu>
        <Menu.Button
          class={cn(
            class,
        ...
        >
          <Menu.Items
            class="nx-absolute nx-right-0 nx-z-20 nx-mt-1 nx-max-h-64 nx-min-w-full nx-overflow-auto nx-rounded-md nx-ring-1 nx-ring-black/5 nx-bg-white nx-py-1 nx-text-sm nx-shadow-lg dark:nx-ring-white/20 dark:nx-bg-neutral-800"
            tabIndex={0}
          >
...
export function Navbar({ flatDirectories, items }: NavBarProps): ReactElement {
  ...
  return (
    <div class="nextra-nav-container nx-sticky nx-top-0 nx-z-20 nx-w-full nx-bg-transparent print:nx-hidden">
      <div
        class={cn(
      ...
      />
      <nav class="nx-mx-auto nx-flex nx-h-[var(--nextra-navbar-height)] nx-max-w-[90rem] nx-items-center nx-justify-end nx-gap-2 nx-pl-[max(env(safe-area-inset-left),1.5rem)] nx-pr-[max(env(safe-area-inset-right),1.5rem)]">
        {config.logoLink ? (
          <Anchor
            href={typeof config.logoLink === 'string' ? config.logoLink : '/'}
            class="nx-flex nx-items-center hover:nx-opacity-75 ltr:nx-mr-auto rtl:nx-ml-auto"
          >
            {renderComponent(config.logo)}
          </Anchor>
        ) : (
          <div class="nx-flex nx-items-center ltr:nx-mr-auto rtl:nx-ml-auto">
            {renderComponent(config.logo)}
          </div>
        )}
        {items.map(pageOrMenu => {
          ...
          if (pageOrMenu.type === 'menu') {
            ...
            return (
              ...
              >
                {menu.title}
                <ArrowRightIcon
                  class="nx-h-[18px] nx-min-w-[18px] nx-rounded-sm nx-p-0.5"
          ...
          return (
            ...
            >
              <span class="nx-absolute nx-inset-x-0 nx-text-center">
                {page.title}
              </span>
              <span class="nx-invisible nx-font-medium">{page.title}</span>
            </Anchor>
          )
        })}
        ...
        {renderComponent(config.search.component, {
          ...
          class: 'nx-hidden md:nx-inline-block mx-min-w-[200px]'
        })}
        ...
        <button
          type="button"
          aria-label="Menu"
          class="nextra-hamburger -nx-mr-2 nx-rounded nx-p-2 active:nx-bg-gray-400/20 md:nx-hidden"
