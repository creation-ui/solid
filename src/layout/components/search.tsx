...
export function Search({
  ...
  return (
    <div class={cn('nextra-search nx-relative md:nx-w-64', class)}>
      {renderList && (
        <div
          class="nx-fixed nx-inset-0 nx-z-10"
      ...
      >
        ...
        >
          {error ? (
            <span class="nx-flex nx-select-none nx-justify-center nx-gap-2 nx-p-8 nx-text-center nx-text-sm nx-text-red-500">
              <InformationCircleIcon class="nx-h-5 nx-w-5" />
              {renderString(config.search.error)}
            </span>
          ) : loading ? (
            <span class="nx-flex nx-select-none nx-justify-center nx-gap-2 nx-p-8 nx-text-center nx-text-sm nx-text-gray-400">
              <SpinnerIcon class="nx-h-5 nx-w-5 nx-animate-spin" />
            ...
            results.map(({ route, prefix, children, id }, i) => (
              <Fragment key={id}>
                ...
                >
                  <Anchor
                    class="nx-block nx-scroll-m-12 nx-px-2.5 nx-py-2"
                    href={route}
                    data-index={i}
