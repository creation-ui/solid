...
export function TOC({ headings, filePath }: TOCProps): ReactElement {
  ...
  return (
    ...
    >
      {hasHeadings && (
        <>
          <p class="nx-mb-4 nx-font-semibold nx-tracking-tight">
            {renderComponent(config.toc.title)}
          </p>
          <ul>
            {items.map(({ id, value, depth }) => (
              <li class="nx-my-2 nx-scroll-my-6 nx-scroll-py-6" key={id}>
                <a
                  href={`#${id}`}
                  class={cn(
                    {
      ...
      {hasMetaInfo && (
        ...
        >
          ...
          {config.toc.backToTop && <BackToTop class={linkClassName} />}
        </div>
      )}
    </div>
  )
